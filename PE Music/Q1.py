...
class PlaylistCLL:
    ...
    def search_by_artist(self, artist_name):
        ...
        while current.next != self.tail:
            if current.info.artist.strip().lower() == artist_name.strip().lower():
        ...
        if current.info.artist.strip().lower() == artist_name.strip().lower():
            found_songs.append(current.info)
        # ---------------------------------------------------------
        return found_songs #vì điều kiện current.next != self.tail sẽ khiến vòng lặp bị đứng khựng lại và bỏ sót mất 2 phần tử cuối cùng (phần tử áp chót và chính phần tử self.tail)
    ...
    def remove_from_playlist(self, song_id):
        ...
        while current.next != self.tail:
            if current.next.info.song_id == song_id:
                tmp = current.next.info
